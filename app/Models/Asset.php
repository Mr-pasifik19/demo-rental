...
class Asset extends Depreciable
{
    ...
    public const MOVEMENTS = 'movementsmodel';
    ...
    public function checkoutToMovement()
    {
        return $this->assignedType() === self::MOVEMENTS;
    }
    /**
    ...
    public function assetLoc($iterations = 1, $first_asset = null)
    {
        if (!empty($this->assignedType())) {
            ...
            if ($this->assignedType() == self::MOVEMENTS) {
                if (($this->assignedTo) && $this->assignedTo->location) {
                    return $this->assignedTo->assignedTo->location;
                }
                return $this->defaultLoc;
            }
        }
    ...
    {
        $result = strtolower(class_basename($this->assigned_type));
        ///
        // $result = str_replace('model', '', $result);
        return $result;
    }
    ...
    /**
     * Get the Multiple image URL of the asset.
     *
     * Check first to see if there is a specific image uploaded to the asset,
     * and if not, check for an image uploaded to the asset model.
     *
     ...
     * @return string | false
     */
    public function getImageUrlForList()    
    {
        if ($this->image && !empty($this->image)) {
            return Storage::disk('public')->url(app('assets_upload_path') . e($this->image));
        } elseif ($this->model && !empty($this->model->image)) {
    ...
    public function advancedTextSearch(Builder $query, array $terms)
    {
        ...
        $query = $query->leftJoin(
            'movements as assets_movements',
            function ($leftJoin) {
                $leftJoin->on('assets_movements.id', '=', 'assets.assigned_to')
                    ->where('assets.assigned_type', '=', MovementsModel::class);
            }
        );

        foreach ($terms as $term) {
            $query = $query->orWhere('assets_movements.movement_number', 'LIKE', '%' . $term . '%');
        }
    ...
    public function scopeAssignedSearch($query, $search)
    {
        ...
        return $query->leftJoin('users as assets_users', function ($leftJoin) {
            ...
            $leftJoin->on('assigned_assets.id', '=', 'assets.assigned_to')
                ->where('assets.assigned_type', '=', self::class);
        })->leftJoin('movements as assets_movements', function ($leftJoin) {
            $leftJoin->on('assets_movements.id', '=', 'assets.assigned_to')
                ->where('assets.assigned_type', '=', MovementsModel::class);
