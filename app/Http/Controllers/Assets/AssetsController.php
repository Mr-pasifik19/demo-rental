...
class AssetsController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $company = Company::all();
        $view = View::make('hardware/edit', compact('company'))
    ...
    public function store(ImageUploadRequest $request)
    {
        ...
        if ($request->has('images')) {
            $asset = $request->handleMultipleImages($asset, $asset_tags);
        }
    ...
    public function edit($assetId = null)
    {
        ...
        $this->authorize($item);
        $company = Company::all();
        return view('hardware/edit', compact('item', 'company'))
            ->with('statuslabel_list', Helper::statusLabelList())
            ->with('statuslabel_types', Helper::statusTypeList());
    }
    ...
    public function show($assetId = null)
    {
        ...
        $settings = Setting::getSettings();
        $images = AssetImage::where('assets_id', $asset->asset_tag)->get();
        ///
        // dd($image->pluck('image'));
        // dd($images);
        if (isset($asset)) {
            ...
            $listImages = Asset::join('assets_image', 'assets.asset_tag', '=', 'assets_image.assets_id')
                ->select('assets_image.id', 'assets.name', 'assets.asset_tag', 'assets_image.image')
                ->where('assets.asset_tag', $asset->asset_tag)
                ->get();
            ...
            return view('hardware/view', compact('asset', 'qr_code', 'settings', 'images', 'listImages'))
    ...
    public function update(ImageUploadRequest $request, $assetId = null)
    {
        ...
        $asset->model_id = $request->input('model_id');
        $asset->order_number = $request->input('order_number');
        $asset->asset_tag = $asset_tag; // Use the null coalescing operator
        $asset->notes = $request->input('notes');
        $asset->physical = '1';

        if ($request->has('images')) {
            $asset = $request->handleMultipleImages($asset, $asset_tag);
        }
    ...
    public function destroy($assetId)
    {
        ...
        $assetImage = AssetImage::where('assets_id', $asset->asset_tag)->get();

        foreach ($assetImage as $imageAsset) {
            try {
                Storage::disk('public')->delete('assets' . '/' . $imageAsset->image);
            } catch (\Exception $e) {
                \Log::debug($e);
            }
            $imageAsset->delete();
    ...
    /**
     * Delete a Image given asset (mark as deleted).
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @param int $assetId
     * @since [v1.0]
     * @return Redirect
     */
    public function destroyAssetImage($id)
    {
        try {
            ...
            if (!$asset) {
                return redirect()->route('hardware.index')
                    ->with('error', trans('admin/hardware/message.assetsimage.notFound'));
            }
            $asset->delete();

            return redirect()->route('hardware.index')->with('success', trans('admin/hardware/message.assetsimage.successDelete'));
        } catch (QueryException $e) {
            // Handle database query exceptions (e.g., foreign key constraints).
            return redirect()->back()->with('error', trans('admin/hardware/message.assetsimage.errorDelete'));
        } catch (\Exception $e) {
