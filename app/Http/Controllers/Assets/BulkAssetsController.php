        ...
        if (($request->filled('purchase_date'))
            ...
            foreach ($assets as $assetId) {
                ...
                if ($request->filled('company_name')) {
                    if ($request->company_name === 'Hantechno Rental Appside_solutions_llc') {
                        Asset::whereRaw('SUBSTRING(asset_tag, 1, 3) = ?', ['20-'])->where('id', $assetId)
                            ->update(['asset_tag' => DB::raw("CONCAT('10-', SUBSTRING(asset_tag, 4))")]);
                    } else {
                        Asset::whereRaw('SUBSTRING(asset_tag, 1, 3) = ?', ['10-'])->where('id', $assetId)
                            ->update(['asset_tag' => DB::raw("CONCAT('20-', SUBSTRING(asset_tag, 4))")]);
                    }
                }
            } // endforeach
