...
class AssetCheckinController extends Controller
{
    ...
    public function create($assetId, $backto = null)
    {
        ...
        return view('movement/checkin', compact('asset'))->with('statusLabel_list', Helper::statusLabelList())->with('backto', $backto);
    }
    ...
    public function store(AssetCheckinRequest $request, $assetId = null, $backto = null)
    {
        ...
        if (is_null($target = $asset->assignedTo)) {
            return redirect()->route('movement.index')->with('error', trans('admin/movement/message.checkin.already_checked_in'));
        }
        $this->authorize('checkin', $asset);
        ...
        if ($asset->save()) {
            ...
            if ((isset($user)) && ($backto == 'user')) {
                return redirect()->route('users.show', $user->id)->with('success', trans('admin/movement/message.checkin.success'));
            }

            return redirect()->route('movement.index')->with('success', trans('admin/movement/message.checkin.success'));
        }
        // Redirect to the asset management page with error
        return redirect()->route('movement.index')->with('error', trans('admin/movement/message.checkin.error').$asset->getErrors());
    }
}
