...
class AssetsController extends Controller
{
    ...
    public function selectlist(Request $request)
    {
        ...
        foreach ($assets as $asset) {
            $assetImg = AssetImage::where('assets_id', $asset->asset_tag)->first();
            ...
            $asset->use_image =   ($assetImg && $assetImg->getImageUrl2()) ? $assetImg->getImageUrl2() : null;
        }
    ...
    public function changeStatusAssetForReturnMovement(Request  $request)
    {
        try {
            $asset = Asset::find($request->idAsset);
            $asset->status_id = $request->status_id;
            $asset->update();

            $status = Statuslabel::find($request->status_id);

            MovementLogs::create([
                'movement_id' => $request->movementId,
                'activity' => 'Change status asset ' . $asset->name . ' to ' . $status->name
            ]);

            return response()->json(Helper::formatStandardApiResponse('success', null, 'Success changes status asset'));
        } catch (\Throwable $e) {
            return response()->json(Helper::formatStandardApiResponse('error', null, 'Failed changes status asset'), 200);
            // return response()->json(Helper::formatStandardApiResponse('success', null, 'Success changes status asset'));
        }
    }
}