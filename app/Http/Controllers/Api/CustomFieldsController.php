...
class CustomFieldsController extends Controller
{
    ...
    public function addNewFieldValues(Request $request)
    {
        try {
            $custom_field = CustomField::find($request->idField);

            if (!$custom_field) {
                return response()->json(Helper::formatStandardApiResponse('error', null, 'Custom field not found'));
            }

            $custom_field->field_values .= "\n" . $request->fields;
            $custom_field->update();  // Use save() instead of update()

            return response()->json(Helper::formatStandardApiResponse('success', $request->fields, 'Success add new field values'));
        } catch (\Throwable $e) {

            return response()->json(Helper::formatStandardApiResponse('error', null, 'Failed to add new field values'), 200);
        }
    }
}
