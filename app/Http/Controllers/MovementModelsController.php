...
/**
 * This class controls all actions related to Movement models for
 * the Snipe-IT Movement Management application.
 *
 * @version    v1.0
 * @author [A. Gianotto] [<snipe@snipe.net>]
 */
class MovementModelsController extends Controller
{
    /**
    ...
    {
        $this->authorize('index', MovementModel::class);

        return view('models/index');
    }

    /**
     * Returns a view containing the Movement model creation form.
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v1.0]
     * @return View
    ...
    {
        $this->authorize('create', MovementModel::class);

        return view('models/edit')->with('category_type', 'movement')
            ->with('depreciation_list', Helper::depreciationList())
            ->with('item', new MovementModel);
    }

    /**
     * Validate and process the new Movement Model data.
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v1.0]
    ...
    /**
     * Returns a view containing the Movement model edit form.
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v1.0]
     * @param int $modelId
    ...
    {
        $this->authorize('update', MovementModel::class);
        if ($item = MovementModel::find($modelId)) {
     ...
     * Movement Model form based on the model ID passed.
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v1.0]
     * @param ImageUploadRequest $request
    ...
    public function update(ImageUploadRequest $request, $modelId = null)
    {
        ...
        if (is_null($model = MovementModel::find($modelId))) {
    ...
    /**
     * Validate and delete the given Movement Model. An Movement Model
     * cannot be deleted if there are associated Movements.
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v1.0]
     * @param int $modelId
    ...
    {
        $this->authorize('delete', MovementModel::class);
        // Check if the model exists
        if (is_null($model = MovementModel::find($modelId))) {
        ...
        if ($model->movements()->count() > 0) {
            // Throw an error that this model is associated with movements
    ...
    /**
     * Restore a given Movement Model (mark as un-deleted)
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v1.0]
     * @param int $modelId
    ...
    public function getRestore($modelId = null)
    {
        ...
        $model = MovementModel::withTrashed()->find($modelId);
    ...
    {
        $this->authorize('view', MovementModel::class);
        $model = MovementModel::withTrashed()->find($modelId);
    ...
    public function getClone($modelId = null)
    {
        $this->authorize('create', MovementModel::class);
        // Check if the model exists
        if (is_null($model_to_clone = MovementModel::find($modelId))) {
            return redirect()->route('models.index')->with('error', trans('admin/models/message.does_not_exist'));
        }
    ...
    {
        return view('models.custom_fields_form')->with('model', MovementModel::find($modelId));
    }
    ...
    public function postBulkEdit(Request $request)
    {
        ...
        if ((is_array($models_raw_array)) && (count($models_raw_array) > 0)) {
            $models = MovementModel::whereIn('id', $models_raw_array)->withCount('movements as movements_count')->orderBy('movements_count', 'ASC')->get();

            // If deleting....
            if ($request->input('bulk_actions') == 'delete') {
                $valid_count = 0;
                foreach ($models as $model) {
                    if ($model->movements_count == 0) {
                        $valid_count++;
                    }
                }
    ...
    public function postBulkEditSave(Request $request)
    {
        ...
        if (count($update_array) > 0) {
            MovementModel::whereIn('id', $models_raw_array)->update($update_array);
    ...
    /**
     * Validate and delete the given Movement Models. An Movement Model
     * cannot be deleted if there are associated Movements.
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v1.0]
    ...
    public function postBulkDelete(Request $request)
    {
        ...
        if ((is_array($models_raw_array)) && (count($models_raw_array) > 0)) {
            $models = MovementModel::whereIn('id', $models_raw_array)->withCount('movements as movements_count')->get();
     ...
     *
     * @param  MovementModel $model
     * @param  array      $defaultValues
     * @return void
     */
    private function assignCustomFieldsDefaultValues(MovementModel $model, array $defaultValues)
    {
        $data = array();
        ...
        foreach ($fieldsets as $fieldset => $validation){
            ...
            // (we are at model level, the rule still applies when creating a new Movement using this model)
     ...
     * @return void
     */
    private function removeCustomFieldsDefaultValues(MovementModel $model)
    {
