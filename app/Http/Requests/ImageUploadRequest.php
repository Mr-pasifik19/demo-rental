...
class ImageUploadRequest extends Request
{
    ...
    /**
     * Handle and store multiple images attached to the request
     * @param SnipeModel $item Item the images are associated with
     * @param string $form_fieldname Field name for images
     * @param string $path Location for uploaded images, defaults to uploads/plural of item type
     * @param string $db_fieldname Database field name for images
     * @return SnipeModel Target asset with attached images
     */
    public function handleMultipleImages($item, $asset_tag, $w = 600, $form_fieldname = 'images', $path = null, $db_fieldname = 'image')
    {
        $type = strtolower(class_basename(get_class($item)));
        if (is_null($path)) {
            $path = str_plural($type);
        ...
        $images = $this->file($form_fieldname); // Assuming 'images' field is an array of files
        // dd($images);
            ...
            if ($image->isValid()) {
                ...
                if (($image->getClientOriginalExtension() !== 'webp') && ($image->getClientOriginalExtension() !== 'svg')) {
                    try {
                        ...
                        $assetImage->image = $file_name;
                        $assetImage->assets_id = $asset_tag;
                        $assetImage->save();
                        Storage::disk('public')->put($path . '/' . $file_name, (string) $upload->encode());
                    } catch (NotReadableException $e) {
                        $validator = \Validator::make([], []);
                    ...
                    // Add the new file name to an array of image names in the database
                    // $item->{$db_fieldname} = $file_name;
                }
            }
        }
        ...
        if ($this->input('image_delete') == '1') {
            foreach ($item->{$db_fieldname} as $imageToDelete) {
                try {
