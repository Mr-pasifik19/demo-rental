...
$(function () {
  ...
  if ($("#createModal").length == 0) {
    $("body").append(
      '<div class="modal fade" id="createModal"></div><!-- /.modal -->'
    );
  }
  ...
  $("#createModal").on("click", "#modal-save", function () {
    $.ajax({
      ...
      success: function (result) {
        ...
        var name =
          result.payload.name ||
          result.payload.first_name + " " + result.payload.last_name;
        if (!id || !name) {
          console.error(
...
function formatDatalistSafe(datalist) {
  ...
  if (datalist.image) {
     ...
     *******************************************************************/
    var img = $(
      "<img src='' style='max-height: 20px; max-width: 30px;' alt=''>"
    );
    // console.warn("Img is: ");
    // console.dir(img);
...
function formatDatalist(datalist) {
  ...
  if (datalist.image) {
    markup +=
      ...
      "' alt='" +
      datalist.tex +
      "' style='max-height: 20px; max-width: 30px;'></div>";
  } else {
