    ...
    <script nonce="{{ csrf_token() }}">
            ...
            return function(value, row) {
                ...
                if ((row.available_actions) && (row.available_actions.update === true)) {
                    actions += '<a href="{{ config('app.url') }}/' + dest + '/' + row.id +
                        '/edit" class="actions btn btn-sm btn-warning" data-tooltip="true" title="{{ trans('general.update') }}"><i class="fas fa-pencil-alt" aria-hidden="true"></i><span class="sr-only">{{ trans('general.update') }}</span></a>&nbsp;';
                ...
                //     if (!currentUrl.includes('hardware')) {
                //         actions +=
                //             '<span data-tooltip="true" title="{{ trans('general.cannot_be_deleted') }}"><a class="btn btn-danger btn-sm delete-asset disabled" onClick="return false;"><i class="fas fa-trash"></i></a></span>&nbsp;';
                //     }
                // }
                if (row.available_actions && row.available_actions.delete !== undefined) {
                    // use the asset tag if no name is provided
                    var name_for_box = row.name || row.asset_tag;
                    ...
                    if (row.available_actions.delete === true) {
                        actions += '<a href="{{ config('app.url') }}/' + dest + '/' + row.id + '" ' +
                            ...
                            ' onClick="return false;">' +
                            '<i class="fas fa-trash" aria-hidden="true"></i><span class="sr-only">{{ trans('general.delete') }}</span></a>&nbsp;';
                    } else if (row.available_actions.delete === false) {
                        actions +=
                            '<span data-tooltip="true" title="{{ trans('general.cannot_be_deleted') }}"><a class="btn btn-danger btn-sm delete-asset disabled" onClick="return false;"><i class="fas fa-trash"></i></a></span>&nbsp;';
                    }
                }
        ...
        function polymorphicItemFormatter(value) {
            ...
            if ((value) && (value.type)) {
                ...
                if (value.type == 'asset') {
                    ...
                    item_destination = 'movement';
                    item_icon = 'fas fa-save';
                }
        ...
        function licenseSeatInOutFormatter(value, row) {
            ...
            if ((row.available_actions.checkout == true) && (row.user_can_checkout == true) && ((!row.asset_id) && (!row
                ...
                return '<a href="{{ config('app.url') }}/licenses/' + row.id +
                    '/checkin" class="btn btn-sm bg-purple" data-tooltip="true" title="Check in this license seat.">{{ trans('general.checkin') }}</a>';
            }
        ...
        function genericCheckinCheckoutFormatter(destination) {
            return function(value, row) {
                ...
                if ((row.available_actions.checkout == true) && (row.user_can_checkout == true) && ((!row.asset_id) && (
                    ...
                    if (row.assigned_to) {
                        return '<a href="{{ config('app.url') }}/' + destination + '/' + row.id +
                            '/checkin" class="btn btn-sm bg-purple" data-tooltip="true" title="Check this item in so it is available for re-imaging, re-issue, etc.">{{ trans('general.checkin') }}</a>';
        ...
        function assetRequestActionsFormatter(row, value) {
            if (value.assigned_to_self == true) {
                ...
                return '<form action="{{ config('app.url') }}/account/request-asset/' + value.id +
                    '" method="POST">@csrf<button class="btn btn-primary btn-sm" data-tooltip="true" title="Request this item">{{ trans('button.request') }}</button></form>';
            }
        ...
        function externalLinkFormatter(value) {
            ...
            if (value) {
                if ((value.indexOf("{") === -1) || (value.indexOf("}") === -1)) {
                    return '<nobr><a href="' + value + '" target="_blank" title="External link to ' + value +
                        '" data-tooltip="true" ><i class="fa fa-external-link"></i> ' + value + '</a></nobr>';
                }
                return value;
            }
        }
        ...
        function deployedLocationFormatter(row, value) {
            if ((row) && (row != undefined)) {
                ...
                return '<a href="{{ config('app.url') }}/locations/' + value.rtd_location.id +
                    '" data-tooltip="true" title="Default Location">' + value.rtd_location.name + '</a>';
            }
        ...
        function departmentNameLinkFormatter(value, row) {
            if ((row.assigned_user) && (row.assigned_user.department) && (row.assigned_user.department.name)) {
                return '<a href="{{ config('app.url') }}/department/' + row.assigned_user.department.id + '">' + row
