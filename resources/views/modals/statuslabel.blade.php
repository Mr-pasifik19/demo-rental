...
<div class="modal-dialog">
    <div class="modal-content">
            ...
            <form action="{{ route('api.statuslabels.store') }}" onsubmit="return false">
                ...
                <div class="dynamic-form-row">
                    <div class="col-md-4 col-xs-12"><label
                            for="modal-type">{{ trans('admin/statuslabels/table.status_type') }}:
                        </label></div>
                    <div class="col-md-8 col-xs-12 required">
                        {{ Form::select('type', $statuslabel_types, '', ['class' => 'select2', 'style' => 'width:90%', 'id' => 'modal-type']) }}
                    </div>
                </div>
            </form>
        </div>
        <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary" id="modal-save" data-action="{{ route('api.statuslabels.store') }}">{{ trans('general.save') }}</button>
        </div>
    </div><!-- /.modal-content -->
</div><!-- /.modal-dialog -->
