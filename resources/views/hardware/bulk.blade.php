...
@section('content')
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            ...
            <form class="form-horizontal" method="post" action="{{ route('hardware/bulksave') }}" autocomplete="off"
                role="form">
                {{ csrf_field() }}
                ...
                <div class="box box-default">
                    <div class="box-body">
                        ...
                        <div class="form-group {{ $errors->has('status_id') ? ' has-error' : '' }}">
                            <label for="status_id" class="col-md-3 control-label">
                                {{ trans('admin/hardware/form.status') }} <code>*Optional</code>
                            </label>
                            <div class="col-md-7">
                                {{ Form::select('status_id', $statuslabel_list, old('status_id'), ['class' => 'select2', 'style' => 'width:100%', 'aria-label' => 'status_id']) }}
                                {!! $errors->first(
                                    'status_id',
                        ...
                        @php
                            $str = '<code>*Optional</code>';
                            $textOptional = htmlspecialchars_decode($str, ENT_NOQUOTES);
                        @endphp
                        @include ('partials.forms.edit.model-select', [
                            'translated_name' => trans('admin/hardware/form.model') . $textOptional,
                            'fieldname' => 'model_id',
                        ])

                        <!-- Default Location -->
                        @include ('partials.forms.edit.location-select', [
                            'translated_name' => trans('admin/hardware/form.default_location') . $textOptional,
                        ...
                        {{-- @include ('partials.forms.edit.supplier-select', [
                            'translated_name' => trans('general.supplier') . ' (Optional)',
                            'fieldname' => 'supplier_id',
                        ]) --}}
                        <!-- Company -->
                        @include ('partials.forms.edit.company-select', [
                            'translated_name' => trans('general.company') . $textOptional,
                    ...
                    <div class="text-right box-footer">
                        <button type="submit" class="btn btn-success"><i class="fas fa-check icon-white"
            ...
            $('#company_select').change(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        var name_company = response.payload.toLowerCase().replace(/\s+/g, '_')
