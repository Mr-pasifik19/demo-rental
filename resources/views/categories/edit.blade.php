...
@section('inputFields')
    ...
    {{-- <div class="form-group {{ $errors->has('category_type') ? ' has-error' : '' }}">
        ...
        <div class="col-md-7 required">
            {{ Form::select('category_type', $category_types, old('category_type', $item->category_type), ['class' => 'select2', 'style' => 'min-width:350px', 'aria-label' => 'category_type', $item->category_type != '' || $item->itemCount() > 0 ? 'disabled' : '']) }}
            {!! $errors->first(
                'category_type',
...
@section('content')
    ...
    @if ($snipeSettings->default_eula_text != '')
        <!-- Modal -->
        <div class="modal fade" id="eulaModal" tabindex="-1" role="dialog" aria-labelledby="eulaModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h2 class="modal-title" id="eulaModalLabel">{{ trans('admin/settings/general.default_eula_text') }}
                        </h2>
                    </div>
