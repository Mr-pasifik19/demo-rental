...
<div>
    ...
    <div class="row">
            ...
            <div class="box">
                <div class="box-body">
                    <div class="alert alert-warning">
                        <strong><i class="fa fa-warning info" aria-hidden="true"></i>
                            {{ trans('general.warning', ['warning' => trans('general.errors_importing')]) }}</strong>
                    </div>
                        ...
                        <div class="col-md-12 table-responsive" style="padding-top: 30px;">
                            <table data-pagination="true" data-id-table="upload-table" data-search="true"
                                ...
                                @foreach ($files as $currentFile)
                                    <tr style="{{ $activeFile && $currentFile->id == $activeFile->id ? 'font-weight: bold' : '' }}"
                                        class="{{ $activeFile && $currentFile->id == $activeFile->id ? 'warning' : '' }}">
                                        <td class="col-md-6">{{ $currentFile->file_path }}</td>
                                        <td class="col-md-3">
                                            {{ Helper::getFormattedDateObject($currentFile->created_at, 'datetime', false) }}
                                        </td>
                                        <td class="col-md-1">{{ Helper::formatFilesizeUnits($currentFile->filesize) }}
                                        </td>
                                        <td class="col-md-1 text-right" style="white-space: nowrap;">
                                            <button class="btn btn-sm btn-info"
                                                wire:click="selectFile({{ $currentFile->id }})">
                                            ...
                                            <button class="btn btn-sm btn-danger"
                                                ...
                                                <i class="fas fa-trash icon-white" aria-hidden="true"></i><span
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    @if ($activeFile->import_type)
                                                        <div class="form-group col-md-12">
                                                            ...
                                                            <h3><i
                                                                    class="{{ Helper::iconTypeByItem($activeFile->import_type) }}"></i>
                                                                Map {{ ucwords($activeFile->import_type) }} Import
                                                                Fields
                                                            </h3>
                                                            <hr style="border-top: 1px solid lightgray">
                                                        </div>
                                                            ...
                                                            @foreach ($activeFile->header_row as $index => $header)
                                                                <div class="form-group col-md-12"
                                                                    ...
                                                                    <div class="col-md-4">
                                                                        ...
                                                                        {{ Form::select(
                                                                            'field_map.' . $index,
                                                                            $columnOptions[$activeFile->import_type],
                                                                            @$field_map[$index],
                                                                            [
                                                        ...
                                                        <div class="form-group col-md-12">
                                                            <div class="col-md-3 text-left">
                                                                <a href="#"
                                                                    wire:click="$set('activeFile',null)">{{ trans('general.cancel') }}</a>
                                                            </div>
                                                            <div class="col-md-9">
                                                                <button type="submit"
        ...
        <div class="col-md-3">
            ...
            <p>The CSV should be comma-delimited and formatted with headers that match the ones in the</p>
            <a href="{{ asset('uploads/file/sample-file-import-asset.csv') }}" class="text-blue">Sample Import Asset</a> &
            <a href="{{ asset('uploads/file/sample-users.csv') }}" class="text-blue">Sample Import People</a>
        </div>
    </div>
</div>
@push('js')
        ...
        $('#fileupload').fileupload({
            dataType: 'json',
                ...
                @this.progress_message =
                    '{{ trans('general.notification_success') }}'; // TODO - we're already round-tripping to the server here - I'd love it if we could get internationalized text here
                @this.progress = 100;
            },
            add: function(e, data) {
                data.headers = {
            ...
            progress: function(e, data) {
                ...
                @this.progress_message = @this.progress +
                    '% Complete'; // TODO - make this use general.percent_complete as a translation, passing :percent as a variable
            },
        ...
        $(function() {
            ...
            $('#upload-table').on('click', '#import', function() {
                ...
                @this.generate_field_map().then(function(mappings_raw) {
                    ...
                    var this_file = @this
                        .file_id; // okay, I actually don't know what I'm doing here.
                    $.post({
                        {{-- I want to do something like: route('api.imports.importFile', $activeFile->id) }} --}}
                        url: "api/v1/imports/process/" +
                            this_file, // maybe? Good a guess as any..FIXME. HARDCODED DUMB FILE
                        ...
                        if ((body) && (body.status) && body.status == 'import-errors') {
                            ...
                            if ((body) && (error in body)) {
                                @this.message = body.error ? body.error :
